...
extern void SetPalette(int idx, int r, int g, int b);
...
{
	//memset(vdp_buffer, 0, 256*192);
}

void video_setpal(int num, int r[], int g[], int b[])
{
	for(int i = 0; i < num; i++)
	{
...
void tms9918_init_palette(tms9918 vdp)
{
    int i;
    for (i = 0; i < 16; i++) {
		vdp->palette[i] = 32+i;//vid_pre_xlat[i];
    }
}
